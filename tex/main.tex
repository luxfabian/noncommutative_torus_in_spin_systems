...
\revise{\begin{align}
    H_\mathrm{HF} = t \sum_{\braket{\vec{i},\vec{j}} \in \mathbb{Z}^{2} \times \mathbb{Z}^2}e^{i a_{\vec{i} \vec{j}}} \ket{ \vec{i}} \bra{ \vec{j}}, % c_\vec{i}^\dagger c_\vec{j},
\end{align}}
...
\end{equation}
This can be used to finally define $\partial_i A = \left. i \rho_{\boldsymbol{\lambda}} (A) \right|_{\boldsymbol{\lambda} \to 1}$.
...
Since we have found that the underlying observable algebra is the noncommutative torus, these \revise{the possible Chern numbers have many interrelations and fulfill}~\cite{Prodan2016}
\begin{equation}
...
When a periodic supercell can be chosen and the magnetic texture is smooth, all of the higher-order Chern numbers can be obtained through integrals over Berry curvature expressions~\cite{Qi2008, Kraus2013, Su2020}.
\revise{In appendix \ref{app:berry_curvature}, we show how excatly the Berry curvature approach is contained in our more general framework.}
...
A detailed overview with all Chern number relations for the cubic hedgehog lattice can be found in table \ref{tab:3q_3d} of the appendix.
...
% \section{The \texorpdfstring{$\Theta$}{Theta}-matrix on the cubic hedgehog lattice}
% \label{app:theta_matrix_cubic_hedgehog}
